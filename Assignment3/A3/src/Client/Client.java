...
public class Client {
	...
	public static void main(String args[]) throws MalformedURLException {
		...
		try {
			URL mtl_url = new URL("http://localhost:8080/montreal?wsdl");
			//target namespace and service name
			QName mtl_name = new QName("http://InterfaceImplimentation/", "Montreal_ClassService");
			mtlService = Service.create(mtl_url, mtl_name);
			
			URL que_url = new URL("http://localhost:8081/quebec?wsdl");
			QName que_name = new QName("http://InterfaceImplimentation/", "Quebec_ClassService");
			queService = Service.create(que_url, que_name);
			
			URL she_url = new URL("http://localhost:8082/sherbrooke?wsdl");
			QName she_name = new QName("http://InterfaceImplimentation/", "Sherbrooke_ClassService");
			sheService = Service.create(she_url, she_name);
			startapplication();
		} catch (Exception e) {
	...
	private static void admin(String username) throws Exception {
		...
		String serverport = decide_server(username);
		if(serverport == "1") {
			startapplication();
			//return;
		}
		
		//ServerObjectInterface obj = (ServerObjectInterface) ServerObjectInterfaceHelper.narrow(.resolve_str(serverport));
		...
		if (optionselected.equals("4")) {
			...
			//boolean n obj = (ServerInterface) registry.lookup("mtlserver");
	...
	private static String decide_server(String username) {
		String serverPort="1";
		...
		if(serverDirection.equals("MTL")) {
			...
			obj = sheService.getPort(webInterface.class);
			serverPort = "she";
		}else {
	...
	private static String new_appointmentType(String patientID) {
		...
		Scanner sc = new Scanner(System.in);
		String appointmentype = sc.nextLine().toUpperCase();
